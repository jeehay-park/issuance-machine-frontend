...
import { deleteKeyIssue } from "../../recoil/atoms/setting";
import Card from "../../components/Layout/Card";

// 키발급코드 설정 삭제
const DeleteKeyIssueConfig: React.FC<{
  children: ReactNode;
  handleRefresh: () => void;
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLDivElement>) => {
    if (selectedRow) {
      try {
        const result = await deleteKeyIssue({
          configType: "KEYISSUE",
        ...
        setResponseMessage("An error occurred while deleting the key issues.");
      }
    }
  };
        ...
        <ModalContainer width="600px">
          <ModalPadding>
            <ModalHeader backgroundColor="var(--layoutBlue)">
              <ModalHeaderTitle>
                <h3 style={{ color: "white" }}>키발급코드 삭제</h3>
              </ModalHeaderTitle>
...
export default DeleteKeyIssueConfig;
